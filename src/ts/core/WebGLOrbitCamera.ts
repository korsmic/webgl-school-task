 ...
 * @class
 */
export default class WebGLOrbitCamera {
  target: any;
  distance: any;
  minDistance: any;
  maxDistance: any;
   ...
   */
  constructor(target: any, option: object = {}) {
    this.target = target;
    ...
    this.target.addEventListener('contextmenu', (event: { preventDefault: () => void; }) => {
   ...
   * マウスボタンが押された際のイベント
   */
  mouseInteractionStart(event: { clientX: number; clientY: number; }) {
    this.isDown = true;
   ...
   * マウスが移動した際のイベント
   */
  mouseInteractionMove(event: { clientX: number; clientY: number; buttons: any; }) {
   ...
   * スクロール操作に対するイベント
   */
  wheelScroll(event: { wheelDelta: any; }) {
